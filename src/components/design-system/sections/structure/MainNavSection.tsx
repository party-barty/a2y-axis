...
function useMainNavOpen() {
  // Read the persisted value in a lazy initializer rather than a mount effect.
  // The upstream template used `useState(true)` + an effect that setState'd on
  // mount, which trips `react-hooks/set-state-in-effect` and renders one frame
  // of the wrong state before correcting.
